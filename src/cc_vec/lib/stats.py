...
def stats(filter_config: FilterConfig, athena_client: CCAthenaClient, crawl: str = "CC-MAIN-2024-33") -> StatsResponse:
    ...
    try:
        ...
        cost_usd = (data_scanned_bytes / (1024**4)) * 5.0  # Athena pricing: $5 per TB scanned
        
        return StatsResponse(
            estimated_records=estimated_records,
            estimated_size_mb=data_scanned_bytes / (1024 * 1024),  # Convert bytes to MB
            estimated_cost_usd=cost_usd,
            data_scanned_gb=data_scanned_bytes / (1024 * 1024 * 1024),  # Convert bytes to GB
            backend="athena",
            crawl_id=crawl,
        )
